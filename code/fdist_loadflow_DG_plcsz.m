function [obj_fun, Ploss,Qloss,tmpv,tmpd,PL,QL,iter,maxerror] = fdist_loadflow_DG_plcsz(dgsz)
global ldata_o ndg
...
bqpu=bq/bKVA;
for tt=1:ndg
    bppu(dgsz(1,2*tt-1))=bppu(dgsz(1,2*tt-1))-dgsz(1,2*tt);
    bqpu(dgsz(1,2*tt-1))=bqpu(dgsz(1,2*tt-1))-0.619744*dgsz(1,2*tt);
end
...
while (maxerror>tolerance)
   ...
   w1=.32;% for power loss
   w2=.32;% for voltage profile
   w3=.32;% for reactive power loss
   w4=.04; % for cost minimization
...
%    disp(obj_fun)
   ...
   obj_fun=w1*Ploss+w2*sum(abs(1-tmpv))+w3*Qloss;%defining objective function
end % end of while
